-- =============================================================================
-- FIX: "new row violates row-level security policy" on term_report_cards
-- Run this ENTIRE file in Supabase Dashboard → SQL Editor → Run
-- =============================================================================

-- Bridge auth user id ↔ teachers.id (class_subjects uses teachers.id)
CREATE OR REPLACE FUNCTION public.report_teacher_record_id(p_user_id uuid)
RETURNS uuid
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT t.id FROM public.teachers t WHERE t.user_id = p_user_id LIMIT 1;
$$;
...
DROP POLICY IF EXISTS "teachers manage own term reports" ON public.term_report_cards;
DROP POLICY IF EXISTS "teachers manage class reports" ON public.term_report_cards;
DROP POLICY IF EXISTS "staff manage term reports" ON public.term_report_cards;
...
-- Secure save function (bypasses RLS after permission check)
...
BEGIN
  ...
  IF NOT public.can_manage_term_report_card(v_uid, v_school_id, v_student_id, v_class_id) THEN
    RAISE EXCEPTION 'Not authorized to save this report card. Ensure you are assigned to this class under Teacher–Class Link.';
  END IF;
  ...
  ELSE
    ...
    INSERT INTO public.term_report_cards (
      school_id, student_id, term_id, class_id, teacher_id, status,
      student_name, class_name, roll_number, class_position, class_student_total_manual,
      term_label, academic_year, attendance_made, attendance_total, conduct, interest,
      club, attitude, teacher_remark, school_closes, reopening_date, next_term,
      teacher_sign_date, head_sign_date, parent_sign_date, admin_comment, rejection_reason,
      subjects, total_score, version, saved_at
    )
    VALUES (
      v_school_id, v_student_id, v_term_id, v_class_id, v_teacher_id, v_status,
  ...
  RETURN jsonb_build_object('id', v_id, 'version', v_new_version, 'status', v_status);
END;
$$;

GRANT EXECUTE ON FUNCTION public.report_teacher_record_id(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_user_school_id(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.teacher_can_access_student_report(uuid, uuid, uuid) TO authenticated;
