-- Payment transaction tracking: columns, idempotency, portal RLS, notifications, audit

-- ---------------------------------------------------------------------------
-- Columns on billing_payments
-- ---------------------------------------------------------------------------
...
-- Idempotency: one gateway reference per successful online payment
...
-- Backfill student_id / parent_id from invoices
...
-- ---------------------------------------------------------------------------
-- Portal RLS: students & parents read own payments
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Enriched view for admin reporting (security invoker)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Notify student, parent, and finance staff on successful payment
-- ---------------------------------------------------------------------------
  ...
  IF v_student.guardian_id IS NOT NULL THEN
    ...
    IF v_parent_user IS NOT NULL AND v_parent_user IS DISTINCT FROM v_student.user_id THEN
      ...
      VALUES (
        ...
        v_title,
        v_body,
        jsonb_build_object('type', 'payment_received', 'payment_id', NEW.id, 'invoice_id', NEW.invoice_id, 'student_id', v_student.user_id)
      );
    END IF;
  END IF;
...
-- ---------------------------------------------------------------------------
-- Delete payment (admin only) — reverses invoice totals
-- ---------------------------------------------------------------------------
