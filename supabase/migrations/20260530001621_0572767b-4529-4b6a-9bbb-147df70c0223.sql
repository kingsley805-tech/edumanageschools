...
BEGIN
  ...
  WHERE c.school_id = coalesce(v_school_id, c.school_id)
    ...
    AND (v_session.gateway_config_id IS NULL OR c.id = v_session.gateway_config_id)
  ...
  IF v_secret = '' OR v_secret IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'error',
      'Paystack secret key not configured. Admin: Payment gateways → save Secret key.');
  END IF;
  ...
  EXCEPTION WHEN OTHERS THEN
    RETURN jsonb_build_object('ok', false, 'error',
      'Could not call Paystack. Enable the http extension in Supabase.');
  END;
  ...
  IF NOT (lower(coalesce(v_body->>'status', '')) IN ('true', 't', '1')
          OR (v_body->'status')::text = 'true') THEN
    RETURN jsonb_build_object('ok', false, 'error',
  ...
  v_invoice_id := coalesce(v_invoice_id, nullif(v_txn->'metadata'->>'invoice_id', '')::uuid);
  v_school_id := coalesce(v_school_id, nullif(v_txn->'metadata'->>'school_id', '')::uuid);
  ...
  SELECT * INTO v_inv FROM public.billing_invoices WHERE id = v_invoice_id AND school_id = v_school_id;
  IF NOT FOUND THEN
    RETURN jsonb_build_object('ok', false, 'error', 'Invoice not found.');
  END IF;

  -- Check if parent_student_links exists (defensive)
  SELECT EXISTS (
  ...
  v_allowed := public.has_role(v_uid, 'admin'::app_role)
    ...
    OR EXISTS (
      SELECT 1 FROM public.students s WHERE s.id = v_inv.student_id AND s.user_id = v_uid
    )
    OR EXISTS (
      SELECT 1
  ...
  IF NOT v_allowed AND v_links_exists THEN
    EXECUTE format($q$
      SELECT EXISTS (
        SELECT 1
        FROM public.parent_student_links psl
        JOIN public.parents p ON p.id = psl.parent_id
        WHERE psl.student_id = %L AND p.user_id = %L
      )
    $q$, v_inv.student_id, v_uid) INTO v_allowed;
  END IF;
  ...
  BEGIN
    INSERT INTO public.billing_payments (
      school_id, invoice_id, student_id, parent_id, amount, currency, method, gateway,
      gateway_ref, paystack_transaction_id, payer_user_id, status, paid_at, payment_context, metadata
    ) VALUES (
      v_school_id, v_invoice_id, v_student_id, v_parent_id, v_amount, v_currency, v_method, 'paystack',
      v_ref, v_txn_id, v_uid, 'paid', now(), 'fees',
  ...
  EXCEPTION
    WHEN undefined_column THEN
      INSERT INTO public.billing_payments (
        ...
        v_school_id, v_invoice_id, v_amount, v_currency, v_method, 'paystack', v_ref, 'paid', now(), 'fees'
      );
  END;
  ...
  SET amount_paid = v_new_paid,
      ...
      status = CASE WHEN v_balance <= 0 THEN 'paid'::public.billing_invoice_status
                    ELSE 'partially_paid'::public.billing_invoice_status END,
...
EXCEPTION
  WHEN unique_violation THEN
    RETURN jsonb_build_object('ok', true, 'duplicate', true);
  WHEN OTHERS THEN
    RETURN jsonb_build_object('ok', false, 'error', coalesce(SQLERRM, 'Failed to record payment.'));
END;
$$;
