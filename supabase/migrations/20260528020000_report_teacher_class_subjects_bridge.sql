-- Bridge auth.uid() (profile/user) with teachers.id used by class_subjects in school-hub.
...
-- Results: teachers assigned via class_subjects (teachers.id)
    ...
    OR EXISTS (
      ...
      WHERE st.id = student_id
        AND (
          ...
          OR cs.teacher_id = public.report_teacher_record_id(auth.uid())
        )
    )
  )
  WITH CHECK (
...
-- Ranking RPC: authorize via teachers.id on class_subjects
...
BEGIN
  ...
  v_teacher_record := public.report_teacher_record_id(v_uid);
  ...
  IF NOT (
    ...
    OR EXISTS (
      ...
      WHERE cs.class_id = p_class_id
        AND (cs.teacher_id = v_uid OR cs.teacher_id = v_teacher_record)
    )
  ) THEN
