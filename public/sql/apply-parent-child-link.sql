-- Parent ↔ student linking (signup + portal)
-- Fixes: guardian_id not set (parents cannot UPDATE students), children invisible via RLS
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object('ok', false, 'error', 'Parent profile not found. Sign out and sign in again, or contact your school.');
  END IF;
  ...
  LOOP
    ...
    IF coalesce((v_preview->>'valid')::boolean, false) IS NOT TRUE THEN
      RETURN jsonb_build_object(
        'ok', false,
        'error', coalesce(v_preview->>'error', 'Invalid admission number: ' || v_num),
        'linked', v_linked
      );
    END IF;
  ...
  SET signup_child_admission_numbers = (
    ...
    FROM unnest(
      coalesce(v_parent.signup_child_admission_numbers, ARRAY[]::text[]) || v_stored
    ) AS x
